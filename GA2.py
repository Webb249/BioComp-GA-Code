...
POPULATION_SIZE = 100
DATA_TWO_POPULATION_SIZE = 64
DATA_TWO_CLASS_LOCATION = 7
DATA_TWO_GENE_SIZE = 6
RULE_LENGTH = 10
GENE_SIZE = 70
...
def create_rules(pop_size, class_location, gene_size):
    data_lines = get_list_of_file_lines('data2.txt')
...
def convert_single_array_solution(individual):
    ...
    for i in range(RULE_LENGTH):
        temp_gene = individual[i * 7:j * 7]
        population_list[i].gene = temp_gene[:6]
        population_list[i].class_number = temp_gene[6]
        j += 1
...
def roulette_selection(population_list):
    offspring = [DataClassSet(None, 0) for _ in range(RULE_LENGTH)]
    temp_fitness_count = 0
    roulette_fitness = 0

    for j in range(RULE_LENGTH):
        roulette_fitness += population_list[j].fitness

    roulette_fitness_random = randrange(roulette_fitness)
    print(roulette_fitness_random)
    for i in range(RULE_LENGTH):
        for k in range(RULE_LENGTH):
            temp_fitness_count += population_list[k].fitness
            if temp_fitness_count >= roulette_fitness_random:
                offspring[i] = population_list[k]
                break
